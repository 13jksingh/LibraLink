...
const Table = ({ items, itemTitle, action }) => {
    return (
        <table className="border-separate w-full text-left border-spacing-y-3">
            <thead>
                <tr>
                    {itemTitle && Object.keys(itemTitle).map(x => (
                        <th className="text-sm font-medium text-[#6b7280] dark:text-[#9ca3af]">
                            {x}
                        </th>
                    )
                    )}
                    {action && (<th className="text-sm font-medium text-[#6b7280] dark:text-[#9ca3af] text-center">
                        Action
                    </th>)}
                </tr>
            </thead>
                        ...
                        <React.Fragment key={x._id}>
                            ...
                            <tr key={x._id}>
                                {itemTitle && Object.keys(itemTitle).map(y => (
                                    <td className="text-sm font-normal">{x[itemTitle[y].alise]}</td>
                                ))}
