...
const IssueBook = ({ id }) => {
    ...
    const { data, error: bookLodingerror, isLoading } = useSWR(
        "/api/student",
        fetcher
    );
    ...
    return (
        <div>
            ...
            {isLoading ? <h1 className="text-center text-xl font-semibold">Loading ...</h1> :
                <form action="/api/issue-book" method="POST" onSubmit={handleIssue} className="flex justify-center items-center gap-2">
